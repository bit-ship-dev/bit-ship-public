...
<div class="wrapper">
  <div>
    ...
    <a class="app-button" href="https://app.bit-ship.dev/companion">Bit-Ship app</a>
  </div>
</div>
</body>
</html>
...
const getApps= (apps) => Object.entries(apps).map(([name, app]) =>
  `<div class="project">
    <h2>${name}</h2>
  ${app.expose.map((expose) =>
    `
      <a href="http://${expose.localHost}">${expose.localHost}</a>
      <p>port: ${expose.port}</p>
      <p>access: ${expose.access}</p>
    `
  )}
  </div>`
) || ''
